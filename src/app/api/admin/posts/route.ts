...
export async function POST(request: Request) {
  ...
  try {
    ...
    // If we're updating a post and the slug changed, delete the old file
...
`;
    ...
    return NextResponse.json({ error: "Failed to save post file" }, { status: 500 });
  }
}
...
export async function DELETE(request: Request) {
  ...
  try {
    ...
    return NextResponse.json({ error: "Failed to delete post file" }, { status: 500 });
  }
}
